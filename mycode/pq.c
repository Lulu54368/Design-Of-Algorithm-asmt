...
void *deletemax(struct pq *pq){
  ...
  for(i = 0; i < pq->count; i++){
    if((pq->priorities)[i] > (pq->priorities)[highestElement]){
      highestElement = i;
    }
  }
  returnVal = (pq->queue)[highestElement];
  /* Delete item from queue by swapping final item into place of deleted
    element. */
  if(pq->count > 0){
    (pq->priorities)[highestElement] = (pq->priorities)[pq->count - 1];
    (pq->queue)[highestElement] = (pq->queue)[pq->count - 1];
    (pq->count)--;
  }
  return returnVal;
}